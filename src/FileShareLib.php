...
function pushSizeToFile($fileMark, $sizeIn, $fsize, $urlDone)
{
...
function getLocationInToken($tokenEnc)
{
    $tk = HTS($tokenEnc);
    if(strstr($tk, "|") === false){
...
function curl_get_contents_timeout($url, $timeout = 0)
{
    $ch = curl_init();
...
function outputLogErrorReportSizeDone($fid, $uid, $tokenEnc, $sizeDone, $ip, $link, $mess)
{
 ...
 * @return void
 */
function updateSizeDownloadRemote($fid, $uid, $tokenEnc, $sizeDone, $ip, $startTime, $endTime)
{
    if(!$sizeDone)
        return;
...
function dl_file_resumable3($file, $filename, $is_resume = TRUE, $fid, $tokenEnc, $uid)
{
